...
module.exports = function (session) {
    ...
    if (options.servers && Array.isArray(options.servers)) {
      ...
      for (var i = 0; i < this.serverNum; i++) {
        ...
        if (client && client.address) {
          clientIdList.push(client.address);
          this.clientIdMap[client.address] = client;
        }
      }
      this.hashRingByClientId = new HashRing(clientIdList);
  ...
  RedisStore.prototype.createRedisClient = function (options) {
    ...
    options.retry_strategy = function (data) {
      ...
      if (data.error && data.error.code === 'ECONNREFUSED') {
        ...
        self.badClientIdMap[clientId] || self.removeClientFromRing(clientId);
      }
    ...
    client.on('error', function (er) {
      ...
      if (self.hashRingByClientId && !self.badClientIdMap[client.address]) {
        self.removeClientFromRing(client.address);
      }
      var badClientIdList = Object.keys(self.badClientIdMap);
      if (badClientIdList.length >= self.serverNum) {
        self.emit('disconnect', er);
      }
    });

    client.on('connect', function () {
      if (self.hashRingByClientId && !self.clientIdMap[client.address]) {
  ...
  RedisStore.prototype.getClient = function (sid) {
    ...
    debug('Redis client hash ring, select client address: %s, sessionId: %s', this.clientIdMap[clientId].address, sid);
  ...
  RedisStore.prototype.addClientToRing = function (client) {
    ...
    this.clientIdMap[client.address] = client;
    delete this.badClientIdMap[client.address];
  ...
  RedisStore.prototype.removeClientFromRing = function (clientId) {
    ...
    this.badClientIdMap[clientId] = 1;
